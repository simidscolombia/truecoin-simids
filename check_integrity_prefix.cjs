...
async function main() {
    const res = await httpsReq(SUPABASE_URL + '/rest/v1/app_settings?key=eq.payment_api_keys&select=value', 'GET', {
        'apikey': SERVICE_ROLE,
        'Authorization': 'Bearer ' + SERVICE_ROLE
    });
    const data = JSON.parse(res.body);
    const keys = JSON.parse(data[0].value);
    console.log("Integrity key starts with:", keys.wompi_integrity.substring(0, 15));
}
