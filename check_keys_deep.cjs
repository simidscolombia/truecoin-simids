...
async function main() {
    const res = await httpsReq(SUPABASE_URL + '/rest/v1/app_settings?key=eq.payment_api_keys', 'GET', {
        'apikey': SERVICE_ROLE,
        'Authorization': 'Bearer ' + SERVICE_ROLE
    });
    const parsed = JSON.parse(res.body);
    const value = JSON.parse(parsed[0].value);
    console.log("Keys available:", Object.keys(value));
    console.log("wompi_integrity:", value.wompi_integrity ? "PRESENT" : "MISSING");
}
