...
async function main() {
    const res = await httpsReq(SUPABASE_URL + '/rest/v1/profiles?email=eq.elkindanielcastillo@gmail.com', 'GET', {
        'apikey': SERVICE_ROLE,
        'Authorization': 'Bearer ' + SERVICE_ROLE
    });
    const profiles = JSON.parse(res.body);
    if (profiles.length === 0) return console.log("No master found");
    const masterId = profiles[0].id;

    console.log("Creating wallet for:", masterId);
    const w = await httpsReq(SUPABASE_URL + '/rest/v1/wallets', 'POST', {
        'apikey': SERVICE_ROLE,
        'Authorization': 'Bearer ' + SERVICE_ROLE,
        'Content-Type': 'application/json'
    }, JSON.stringify({ user_id: masterId, balance_tc: 0 }));

    console.log("Status:", w.status, w.body);
}
main();
