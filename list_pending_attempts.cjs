...
async function main() {
    const res = await httpsReq(SUPABASE_URL + '/rest/v1/registration_attempts?status=eq.pending', 'GET', {
        'apikey': SERVICE_ROLE,
        'Authorization': 'Bearer ' + SERVICE_ROLE
    });
    console.log("Últimos intentos PENDIENTES:");
    const attempts = JSON.parse(res.body);
    attempts.forEach(a => {
        console.log(`- Ref: ${a.reference}, Nombre: ${a.full_name}, Email: ${a.email}, Creado: ${a.created_at}`);
    });
}
