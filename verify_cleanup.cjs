...
async function main() {
    const res = await httpsReq(SUPABASE_URL + '/rest/v1/profiles?select=id,full_name,email,referral_code', 'GET', {
        'apikey': SERVICE_ROLE,
        'Authorization': 'Bearer ' + SERVICE_ROLE
    });
    const profiles = JSON.parse(res.body);
    console.log("Perfiles actuales:");
    profiles.forEach(p => console.log(`- ${p.full_name} (${p.email}) [${p.referral_code}]`));
}
